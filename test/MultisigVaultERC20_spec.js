...
contract("MultisigVaultERC20", accounts => {
  beforeEach(async function () {
    ...
    this.MultisigVaultERC20 = await MultisigVaultERC20.new(2, [this.party1, this.party2], this.EursTestToken.address, { from: this.party1 });
  ...
  describe('balance ready', function() {
    ...
    describe('party 1 approved', function () {
      ...
      describe('party2 regress', function () {
        it('not allowed', async function () {
          await assertRevert(
            this.MultisigVaultERC20.regress(
              this.destination,
              this.amount, // 1 ETH
              { from: this.party2 }
            )
    ...
    describe('party1 regress', function () {
      beforeEach(async function () {
        this.tx = await this.MultisigVaultERC20.regress(
          this.destination,
          this.amount, // 1 ETH
          { from: this.party1 }
        )
